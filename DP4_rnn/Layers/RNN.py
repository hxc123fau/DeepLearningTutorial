...
class RNN():
    ...
    def forward(self,input_tensor):
        # print('rnn_hidden',self.hidden_state)
        ...
        if self.same_sequence==False:
            ...
            # if isinstance(self.hidden_state,list) :
                self.hidden_state = np.zeros((self.batch_size + 1, self.hidden_size))
            else:
                # self.last_iter_hidden_state=np.load('last_iter_hidden_state.npy')
                # print('self.last_iter_hidden_state',self.last_iter_hidden_state)
                self.hidden_state[0]=self.last_iter_hidden_state*self.same_sequence

        y_out=np.zeros((self.batch_size,self.output_size))

        for b in range (self.batch_size):
            # print('222',self.input_tensor[b].shape,self.hidden_state[b].shape)
            new_input=np.hstack((np.expand_dims(self.hidden_state[b],0),np.expand_dims(self.input_tensor[b],0)))
            # print('self.input_tensor[b]',self.input_tensor[b])
            # print('new_input',new_input)
            self.hidden_state[b+1]=np.tanh(self.fcl_h.forward(new_input))
            # print('self.hidden_state[b+1]',self.hidden_state[b+1].shape)
            y_out[b]=(self.fcl_y.forward(np.expand_dims(self.hidden_state[b + 1],0)))

        # np.save('last_iter_hidden_state.npy',self.hidden_state[-1])
        self.last_iter_hidden_state=self.hidden_state[-1]
        # print('self.hidden_state[self.batch_size+1]',self.hidden_state[-1])
    ...
    def backward(self,error_tensor):
        ...
        error_h=np.zeros((1,self.hidden_size))  # backward里最后的需要初始置零
        # error_h=self.hidden_state[-1]
        ...
        for b in reversed(range(self.batch_size)):
            ...
            error_h = error_hx[:, 0:self.hidden_size]   # hidden我加在前面了
            ...
            steps+=1
            # gradient_weights_hx = self.gradient_weights_hx
            # gradient_weights_y = self.gradient_weights_y
    ...
    def get_gradient_weights(self):
       ...
       # print('rnn_gra',self.gradient_weights_y.shape,self.gradient_weights_h.shape)
