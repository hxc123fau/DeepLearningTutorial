...
class LSTM():
    ...
    def forward(self,input_tensor):
        ...
        for b in range(self.batch_size):
            # print('np.expand_dims(self.input_tensor[b],0)',np.expand_dims(self.hidden_state[b],0) )
            ...
            # y_out[b] = self.sigmoid(np.expand_dims(self.hidden_state[b + 1], 0))
    ...
    def backward(self,error_tensor):
        ...
        for b in reversed(range(self.batch_size)):
            # gradient_y_sig = (self.error_tensor[b]-self.error_tensor[b]**2)                 # 对sigmoid的求导
            # print('gradient_y_sig',gradient_y_sig.shape,gradient_y_sig)
            gradient_y_fcl_h=self.fcl_y.backward(np.expand_dims(self.error_tensor[b],0))
            # gradient_y_h=gradient_y_sig_h+back_hidden_state[b+1]
            gradient_y_h = gradient_y_fcl_h + hidden
            # print('gradient_y_h',gradient_y_h)
            ...
            gradient_ct_sf=self.cell_state[b]     # C(t-1) 但是反向的时候是从右边开始的，我的b从大到小
            ...
            # gradient_c_hat_x =(1-np.tanh(self.c_hat_t[b]**2))
            gradient_c_hat_x = (1 - self.c_hat_t[b] ** 2)

            # gradient_y_x = gradient_y_h * (gradient_h_o * gradient_o_x + gradient_h_ct_all + (
            #             gradient_ct_f * gradient_f_x + gradient_ct_i * gradient_i_x +
            #             gradient_ct_c_hat * gradient_c_hat_x))
            ...
            # back_hidden_state[b]=self.hx[b,0:self.hidden_size]
            ...
            if steps <= self.bptt_length:
                ...
                self.weights_f = self.fcl_f.get_weights()   #不用lstm中的get_weights用fcl中自身的就好
                self.weights_i=self.fcl_i.get_weights()
                self.weights_c_hat=self.fcl_c_hat.get_weights()
                self.weights_o=self.fcl_o.get_weights()
                # print('weights_all',self.weights_y.shape,self.weights_o.shape)
    ...
    def initialize(self,weights_initializer,bias_initializer):
        ...
        # self.fcl_hx.initialize(weights_initializer, bias_initializer)
    ...
    def get_weights(self):
        ...
        # print('wf,wi,wch,wo',wf.shape,wi.shape,wc_hat.shape,wo.shape)

        weights=np.hstack((wf,wi,wc_hat,wo))
        # print('stack_weights',weights.shape)
        return weights
