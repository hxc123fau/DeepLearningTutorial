...
class BatchNormalization:
    ...
    def forward(self,input_tensor):
        ...
        # feature_size=self.input_tensor.shape[1]
        ...
        if (len(self.channels) !=0 ):
            ...
            # new_tensor=np.zeros_like(self.input_tensor)
            new_tensor0=self.input_tensor
            # print('new_tensor0',new_tensor0.shape)
            new_tensor1=new_tensor0.reshape(input_shape[0],input_shape[1],input_shape[2]*input_shape[3])
            # print('new_tensor1', new_tensor1.shape)
            new_tensor2=np.transpose(new_tensor1,(0,2,1))
            # print('new_tensor2', new_tensor2.shape)
            self.new_tensor=new_tensor2.reshape((input_shape[0]*input_shape[2]*input_shape[3],input_shape[1]))
            # print('new_tensor',self.new_tensor.shape)
            # self.new_tensor = new_tensor2.reshape(1800, 2)
        else:
            self.new_size=(batch_size,feature_size)
            self.new_tensor = self.input_tensor

        # self.weights=np.ones((1,self.input_tensor.shape[1]))
        # self.bias=np.zeros((1,self.input_tensor.shape[1]))
        ...
        for i in range( self.new_tensor.shape[1]):
            # print(' self.input_tensor.shape[1]', self.input_tensor.shape[1])
            mean_i [0,i]= np.mean(self.new_tensor[:,i])  #
            stand_var_i [0,i]= np.sqrt(np.var(self.new_tensor[:,i]))

            # print(' mean[0],bar[0]', mean[i], stand_var[i],self.input_tensor.shape)
        ...
        # self.y_hat=self.y_hat.reshape(input_tensor.shape)

        # print('y_hat11',self.y_hat)
        if (len(self.channels) != 0):
    ...
    def backward(self,error_tensor):
        ...
        if (len(self.channels) != 0):
            ...
            self.new_error=self.error_input #其实不用reshape也可以
        ...
        # gradient_x=gradient_x.reshape(error_tensor.shape)
        ...
        if (len(self.channels) != 0):
            ...
            # print(self.temp1,self.temp2,self.temp3)
            # self.gradient_x_out=gradient_x.reshape(np.shape(self.error_input))
        else:
            self.gradient_x_out = gradient_x
    ...
    def get_gradient_weights(self):
        # print('self.error_input,self.x_hat',self.error_input.shape,self.x_hat.shape)
